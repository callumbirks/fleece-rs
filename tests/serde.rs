...
fn serde() {
    ...
    assert!(matches!(
        ...
        fleece::Error::Serialize(fleece::error::SerializeError::ValueNotCollection)
    ));
...
fn serde_shared_keys() {
    ...
    let scope = fleece::to_bytes_with_shared_keys(&students).expect("Error serializing");
    let de_students: Vec<Student> =
        fleece::from_bytes(&scope.data().unwrap()).expect("Error deserializing");
